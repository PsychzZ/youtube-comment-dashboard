...
logistic_model = LogisticRegression(max_iter=1000, solver='lbfgs') # Initializing logistic regression model with 1000 iterations
...
# Save the model
filename = 'finalized_model.sav'  # Defining filename for saving the trained model
pickle.dump(logistic_model, open(filename, 'wb'))  # Serializing and saving the trained model to a file

pickle.dump(vectorizer, open('vectorizer.sav', 'wb'))  # Saving the vectorizer for future use
